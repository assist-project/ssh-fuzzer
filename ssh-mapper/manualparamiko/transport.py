...
from manualparamiko.kex_curve25519 import KexCurve25519
from manualparamiko.kex_gex import KexGex, KexGexSHA256
from manualparamiko.kex_group1 import KexGroup1
from manualparamiko.kex_group14 import KexGroup14, KexGroup14SHA256
from manualparamiko.kex_group16 import KexGroup16SHA512
...
from manualparamiko.util import (
...
#time params
    def fuzz_userauth_pw_nok(self):
        ...
        return self.read_multiple_responses(timeout=self.auth_pw_nok_to, total_timeout=self.auth_pw_nok_to_total)
    ...
    def fuzz_userauth_pw_ok(self):
        ...
        return self.read_multiple_responses(timeout=self.auth_pw_ok_to, total_timeout=self.auth_pw_ok_to_total)
...
#set user and pwd
    def fuzz_userauth_pw(self, ok):
        ...
        if ok:
            ...
            self.auth_handler.password = 'stud$nt'  # Just an example, make sure this user/pass is denied!
        self.auth_handler.custom_parse_service_request()
...
#time param
    ...
    def read_multiple_responses(self, timeout=None, total_timeout=None):
        ...
        while True:
            ...
            if response == '':
                ...
                if MSG_NAMES[ptype] in last_msg and last_msg[-1] == '*':
                    # Alright, we are dealing with a repeating pattern here. Either add * or skip.
            ...
            if total_timeout is None or time.time()-start > total_timeout or response == 'NO_CONN':
                break
        ...
        # BitVisor case of buffering every output after a second kexinit, and sending all output once the key exchange is finished
        if self.buffer_after_newkey is True and response.find("KEX31+NEWKEYS+") == 0: #response starts with this
            response = "KEX31+NEWKEYS+BUFFERED"
            # unfortunately, all the buffered responses might take more than TO value to read
    ...
    def _activate_outbound(self, update=True):
        """switch on newly negotiated encryption parameters for
        outbound traffic"""
        m = Message()
        m.add_byte(cMSG_NEWKEYS)
